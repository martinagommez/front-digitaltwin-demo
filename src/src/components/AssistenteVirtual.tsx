...
function AssistenteVirtual() {
	...
	const fetchPlugins = async () => {
		try {
			...
			console.log('setupApi fetchPlugins',setupApi);
			setPluginRequest(response.data);
			if (plugins?.NumberOfPlugins === 1) {
				const activePlugin = plugins.PluginList[0];
				setActivedPlugin(activePlugin);
				setInputEnable(true);
				setPluginType(activePlugin.PluginType);
				console.log("Só tenho um plugin!");
				console.log("activedPlugin if",activePlugin);
			} else {
				console.log("Tenho mais que um plugin!");
				console.log("activedPlugin else",plugins.PluginList);
	...
	// Fetch dos plugins
	useEffect(() => {
		if (setupApi && !activedPlugin) {
			console.log("ESTOU A CORRER O USEEFFECT");
			fetchPlugins();
		}
	}, [setupApi, activedPlugin]);

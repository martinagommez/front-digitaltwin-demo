		...
		const updateAppConfig = async () => {
			try {
				...
				if (clientData.enableFeatures) {
					setFeaturesStates({
						enableNewChat: clientData.enableFeatures.enableNewChat ?? true,
						enableDarkMode: clientData.enableFeatures.enableDarkMode ?? true,
						enableDebugMode: clientData.enableFeatures.enableDebugMode ?? true,
						enableAudio: clientData.enableFeatures.enableAudio ?? true,
						enableLanguages: clientData.enableFeatures.enableLanguages ?? true,
						pluginsTitleOption: clientData.enableFeatures.pluginsTitleOption ?? true,
					});
				}
		...
		const updatedAppConfig = async () => {
			try {
				...
				if (language) {
					...
					setTitle(clientData.title || 'Title');
					setTabText(clientData.tabText || 'Homepage');
					setPluginsTitle(clientData.pluginsTitle || 'Plugins');
					setButtonLabels({
						...
						<div className="flex items-center gap-2 flex-shrink-0">
							<div className="hidden md:flex space-x-2">
								...
								{featuresStates.enableNewChat && (
									<div className="relative group">
										<button
											title={buttonLabels.newChatButton}
								...
								{featuresStates.enableDarkMode && (
									<div className="relative group">
										<button
											title={darkMode ? buttonLabels.lightModeButton : buttonLabels.darkModeButton}
			...
			>
				<div className="p-4 flex flex-col space-y-4">
					...
					{featuresStates.enableDarkMode && (
						...
						>
							{darkMode ? (
								<FaSun className="text-yellow-500 w-5 h-5 flex-shrink-0" />
							) : (
				...
				{featuresStates.enableLanguages && (
					<div className='border-t-2 border-gray-400 m-4 mt-1'>
						...
						<div 
							...
							style={{ width: 'full', maxHeight: 'calc(100vh - 320px)' }}
						>
							{availableLanguages.map(({ code, label, flag }) => (
								<button
									key={code}
